...
def animal_detail(request, id_animal):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if nouveau_lieu.disponibilite == 'libre' and check_status(animal, nouveau_lieu.id_equip):
            refresh_status(animal, nouveau_lieu.id_equip)
            form.save()
            ...
            nouveau_lieu.save()
            message = f'{animal.id_animal} est maintenant à {nouveau_lieu.id_equip} et il est {animal.etat}.'
            error = False
        else:
            # Error message
            message = 'Error: Cette modification ne peut pas être realisée.'
            error = True
...
def check_status (animal, n_lieu):
...
def refresh_status (animal,lieu):
    if lieu == 'litière':
        animal.etat = 'affamé'
    elif lieu == 'mangeoire':
        animal.etat = 'repus'
